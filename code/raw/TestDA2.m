...
y = zeros(1,N);
z = zeros(1,N);
lag = 300; % lag for computing the ACF
...
gamma_mean_trace1 = gamma_mean_trace;
...
%     mu_s(:,i) = gamma_mean_trace(:,i) + C_temp * (mu_s(:,i+1) - a0 * dt - ( eye(n) + a1 * dt) * gamma_mean_trace(:,i)); % update the smoother mean
    mu_s(:,i) = mu_s(:,i+1) + (-a0 - a1 * mu_s(:,i+1) + b1 * b1' * inv(gamma_cov_trace(:,:,i+1)) * (gamma_mean_trace(:,i+1) - mu_s(:,i+1))) * dt;
...
%         - Y_Sampling_Save(:,i+1,j)) * dt + b1 * randn(n,1) * sqrt(dt); % Backward sampling; the sampled trajectory has random noise
        Y_Sampling_Save(:,i,j) = Y_Sampling_Save(:,i+1,j) - mu_s(:,i+1) + mu_s(:,i) - (a1 + b1 * b1' * inv(gamma_cov)) * (Y_Sampling_Save(:,i+1,j) - mu_s(:,i+1)) * dt + b1 * randn(n,1) * sqrt(dt); 
    end
...
for k = 1:s_n
    ...
    for i = N-1:-1:1
        ...
        R_s_temp = gamma_cov + C_temp * (R_s_temp - (eye(n) + a1 * dt) * gamma_cov * (eye(n) + a1 * dt)' - b1 * b1' * dt) * C_temp';   
        R_s(i) = R_s_temp; % update the smoother covariance (this line and the above two lines)  
        Z_Sampling_Save(i,k) = Z_Sampling_Save(i+1,k) - mu_s(i+1) + mu_s(i) - (a1 + b1 * b1' * inv(gamma_cov)) * (Z_Sampling_Save(i+1,k) - mu_s(i+1)) * dt + b1 * randn(n,1) * sqrt(dt); 
    end
    u_post_mean_all(k,:) = mu_s;
    u_post_cov_all(k,:) = R_s;    
end
z_sampled = Z_Sampling_Save;
z_mean = mean(u_post_mean_all);%mean(z_sampled');%
...
z_sampled = Z_Sampling_Save(1,:,:);
z_mean2 = mean(u_post_mean_all);%mean(z_sampled');%
...
RMSE_I = sqrt(sum((z - u_post_mean_one_step(2,:)).^2)/N);
RMSE_III = sqrt(sum((z - z_mean2).^2)/N);
disp('RMSEs of Method I and Method III')
disp([RMSE_I, RMSE_III])
...
gamma_mean_trace = u_post_mean;
gamma_cov_trace = u_post_cov;
gamma_mean_trace3 = gamma_mean_trace;
mu_s = zeros(1,N); % Save the posterior mean in smoothing
R_s = zeros(1,N); % Save the posterior covariance in smoothing
...
post_lower_one_step = u_post_mean_one_step(2,:) - 2 * transpose(sqrt(squeeze(u_post_cov_one_step(2,2,:))));
patch([tt,tt(end:-1:1)],[post_lower_one_step,post_upper_one_step(end:-1:1)],'r','facealpha',0.2,'linestyle','none');
h3 = plot(dt:dt:N*dt, z_mean3,'c','linewidth',2);
post_upper = z_mean3 + 2 * sqrt(z_var3);
post_lower = z_mean3 - 2 * sqrt(z_var3);
patch([tt,tt(end:-1:1)],[post_lower,post_upper(end:-1:1)],'c','facealpha',0.2,'linestyle','none');
box on
set(gca,'fontsize',12)
legend([h1,h2,h3],'Truth','Method I','Method IV')


RMSE_I = sqrt(sum((z - u_post_mean_one_step(2,:)).^2)/N);
RMSE_IV = sqrt(sum((z - z_mean3).^2)/N);
disp('RMSEs of Method I and Method IV')
disp([RMSE_I, RMSE_IV])