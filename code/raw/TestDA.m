...
% rng(1);
% N = 100000; % total nubmer of time steps
% dt = 0.005; % time step
% x = zeros(1,N);
% y = zeros(1,N);
% z = zeros(1,N);
...
% disp(corcf(1,2))
% 
% % disp('Correlation between x and z')
% % corcf = corrcoef(x,z);
% % disp(corcf(1,2))
% 
...
% 
%     % forward sampling
%     premat = (R0*A1') * InvBoB * (A1*R0');
%     for j= 1:s_n
%         Y_Sampling_Save(:,i,j) = Y_Sampling_Save(:,i-1,j) + (mu-mu0) + (a1 - premat / R0) * (Y_Sampling_Save(:,i-1,j) - mu0) * dt ...
%             + sqrtm(b1*b1' + premat) * randn(n,1) * sqrt(dt);
%     end
% 
%     u_post_mean(:,i) = mu;
%     u_post_cov(:,:,i) = R;
...
% %     for j = 1:s_n
% %         Y_Sampling_Save(:,i,j) = Y_Sampling_Save(:,i+1,j) + (-a0 - a1 * Y_Sampling_Save(:,i+1,j)) * dt + b1 * b1' / (gamma_cov) * (gamma_mean_trace(:,i) ... 
...
%     % gamma_cov_trace = u_post_cov;
%     % 
%     u_post_mean_all(k,:) = u_post_mean;
%     u_post_cov_all(k,:) = u_post_cov;
% 
...
n_spin = 1;
RMSE_I = sqrt(sum((z(1,n_spin:end) - u_post_mean_one_step(2,n_spin:end)).^2)/N);
RMSE_II = sqrt(sum((z(1,n_spin:end) - z_mean(1,n_spin:end)).^2)/N);
disp('RMSEs of Method I and Method II')
disp([RMSE_I, RMSE_II])

% plot mixture of Gaussian pdf
iN = 15600;
% Define the range of x values over which to plot the PDF
x_values = linspace(min(u_post_mean_all(:, iN)) - 3*max(sqrt(u_post_cov_all(:, iN))), ...
                    max(u_post_mean_all(:, iN)) + 3*max(sqrt(u_post_cov_all(:, iN))), 1000);

% Initialize the PDF values to zero
pdf_values = zeros(size(x_values));
weights = ones(1, s_n) / s_n;
% Calculate the PDF of the mixture
for i = 1:s_n
    % Mean and standard deviation of the i-th Gaussian component
    mu_i = u_post_mean_all(i, iN);
    sigma_i = sqrt(u_post_cov_all(i, iN));

    % Update the PDF values by adding the PDF of the i-th Gaussian component
    pdf_values = pdf_values + weights(i) * normpdf(x_values, mu_i, sigma_i);
end

fitted_pdf = normpdf(x_values, z_mean(iN), sqrt(z_var(iN)));
fitted_pdf_I = normpdf(x_values, u_post_mean_one_step(2, iN), sqrt(u_post_cov_one_step(2, 2, iN)));

% Plot the PDF of the mixture
figure(5)
plot(x_values, pdf_values, 'LineWidth', 2, 'DisplayName', 'Mixture of Gaussians');
hold on;
plot(x_values, fitted_pdf, 'r--', 'LineWidth', 2, 'DisplayName', 'Fitted Gaussian');
hold on;
plot(x_values, fitted_pdf_I, 'g:', 'LineWidth', 2, 'DisplayName', 'Method I');
xlabel('x');
ylabel('Probability Density');
legend('show');
title(sprintf('PDF at t=%.1f', iN*dt));
grid on;
...
h4 = plot(dt:dt:N*dt, z_mean,'g','linewidth',2);
patch([tt,tt(end:-1:1)],[post_lower,post_upper(end:-1:1)],'g','facealpha',0.2,'linestyle','none');

h3 = plot(dt:dt:N*dt, z_mean2,'k','linewidth',2);
post_upper2 = z_mean2 + 2 * sqrt(z_var2);
post_lower2 = z_mean2 - 2 * sqrt(z_var2);
patch([tt,tt(end:-1:1)],[post_lower2,post_upper2(end:-1:1)],'k','facealpha',0.2,'linestyle','none');
box on
set(gca,'fontsize',12)
legend([h1,h2,h4,h3],'Truth','Method I','Method II','Method III')
